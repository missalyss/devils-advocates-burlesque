...
function appendArticle() {
    for (var i = 0; i < cast.length; i++) {
        $('.performer-main').append(rowOpen + pictureColOpen + `<img src="${cast[i].img}" alt="${cast[i].imgAlt}">` + divClose + articleColOpen + `<h3>${cast[i].name}</h3>` + `<p>${cast[i].about}</p>` + divClose + divClose + `<div class="divider"></div>` + br)
    }
}
